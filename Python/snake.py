...
def game_loop(window, game_speed):
    # setup inicial
    curses.curs_set(0) #retira o cursor na tela de apresentacao
    snake = [
        [10, 15],
        [10, 14],
        [10, 13],
        [10, 12],
    ]# Coordenadas do curses [y, x]
    ...
    finished_game(score, window)
    
    
def finished_game(score, window):
    height, width = window.getmaxyx()
    final_message = f'Voce perdeu! Coletou {score} frutas!'
    y = int(height / 2)
    x = int((width - len(final_message)) / 2 )
    window.addstr(y, x, final_message)
    window.refresh()
    time.sleep(3)
...
def draw_screen(window):
    window.clear()
    window.border(0) #cria uma borda nos limites da janela
...
def get_new_direction(window, timeout):
    ...
    if direction in [curses.KEY_UP,curses.KEY_DOWN, curses.KEY_RIGHT, curses.KEY_LEFT]:
        return direction
    return None
...
def select_difficulty():
    ...
    while True:
        answer = input('Selecione o nivel de dificuldade de 1 a 5:')
        game_speed = difficulty.get(answer)
        if game_speed is not None:
            return game_speed
        print('Escolha o nivel de dificuldade de 1 a 5:')           
